...
kubectl --context="${CLUSTER}" create ns "${ENVIRONMENT}" 2>/dev/null || true
kustomize build . | kubectl --context="${CLUSTER}" apply -f -

popd > /dev/null

rm -rf "$tmp_dir"

kubectl --context="${CLUSTER}" -n "${ENVIRONMENT}" rollout status deploy/quarkus-app-${ENVIRONMENT} --timeout=180s
